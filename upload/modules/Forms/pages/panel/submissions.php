...
if (!isset($_GET['view'])) {
    ...
    if (!isset($_GET['action'])) {
                    ...
                    if ($any_changes == true) {
                        ...
                        if (!empty(Input::get('content'))) {
                            ...
                            if (isset($new_status)&& $new_status->exists()) {
                                $content .= "\n\n" . str_replace(['{status}', '{new_status}'], [strip_tags($status->data()->html), strip_tags($new_status->data()->html)], $forms_language->get('forms', 'updated_submission_status'));
                            }
                        } else {
                            // No comment, just status change
                            $content = str_replace(['{status}', '{new_status}'], [strip_tags($status->data()->html), strip_tags($new_status->data()->html)], $forms_language->get('forms', 'updated_submission_status'));
                        }
                        
                        HookHandler::executeEvent('updatedFormSubmissionStaff', [
                            'event' => 'updatedFormSubmissionStaff',
                            'user_id' => $user->data()->id,
                            'username' => $user->getDisplayname(),
                            'content' => str_replace(['{x}', '{y}'], [$form->data()->title, $user->getDisplayname()], $forms_language->get('forms', 'updated_submission_text')),
        ...
        if (empty($submission->data()->content)) {
            ...
            $fields = DB::getInstance()->query('SELECT name, value, type FROM nl2_forms_replies_fields LEFT JOIN nl2_forms_fields ON field_id=nl2_forms_fields.id WHERE submission_id = ?', [$submission->data()->id])->results();
